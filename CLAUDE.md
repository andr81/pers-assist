...
**КРИТИЧЕСКИ ВАЖНО:** В начале каждой сессии получай актуальную дату и день недели через системную команду:

```bash
date "+%Y-%m-%d %A %H:%M"
```

**Пример вывода:** `2026-01-05 Monday 10:35`

### Правила:
- ⚠️ **ЕДИНСТВЕННЫЙ НАДЁЖНЫЙ ИСТОЧНИК ДАТЫ — СИСТЕМНАЯ КОМАНДА `date`**
- ⚠️ **НЕ ИСПОЛЬЗУЙ Google Calendar API, timeapi.io или другие внешние API для получения текущей даты!**
- ⚠️ **НИКОГДА НЕ УГАДЫВАЙ ДНИ НЕДЕЛИ — ТОЛЬКО ИЗ ВЫВОДА `date`**
- ⚠️ **ПЕРЕД ЛЮБОЙ РАБОТОЙ С ДАТАМИ — ВЫПОЛНИ `date "+%Y-%m-%d %A"`**
- При расчёте будущих/прошлых дат используй `date -d`:
  - Завтра: `date -d "tomorrow" "+%Y-%m-%d %A"`
  - Через 3 дня: `date -d "+3 days" "+%Y-%m-%d %A"`
  - Пятница этой недели: `date -d "next friday" "+%Y-%m-%d %A"`
- **Пример:** `date` вернул `2026-01-05 Monday` → значит:
  - Вт 06.01, Ср 07.01, Чт 08.01, Пт 09.01, Сб 10.01, Вс 11.01
- Timezone системы настроен на Europe/Minsk
...
1. **Текущая дата и время (ТОЛЬКО через системную команду!):**
```bash
date "+%Y-%m-%d %A %H:%M"
```
